...
const FirestorePopulator = () => {
  ...
  const clearProjects = async () => {
    try {
      ...
      addLog(
        `🗑️ Clearing ${snapshot.docs.length} existing projects...`,
        'info'
      );
  ...
  const populateProjects = async () => {
    try {
      addLog(
        `📦 Adding ${projectsData.length} projects to Firestore...`,
        'info'
      );
      ...
      for (let i = 0; i < projectsData.length; i++) {
        ...
        try {
          ...
          addLog(
            `✅ Added project ${i + 1}/${projectsData.length}: ${project.title}`,
            'success'
          );
        } catch (error) {
          addLog(
            `❌ Error adding project "${project.title}": ${error.message}`,
            'error'
          );
          throw error;
        }
  ...
  const handlePopulate = async () => {
    ...
    try {
      ...
      addLog(
        `📊 Summary: ${projectsData.filter(p => p.isFeatured).length} featured, ${projectsData.filter(p => !p.isFeatured).length} grid projects`,
        'info'
      );
      addLog(
        '✅ Your portfolio will now load projects from Firestore!',
        'success'
      );
    } catch (error) {
      setStatus('Failed!');
    ...
    <div className='max-w-4xl mx-auto p-6 bg-primary-900 text-gray-100'>
      <div className='bg-primary-800/50 rounded-lg p-6 border border-primary-700/50'>
        ...
        <p className='text-gray-300 mb-6'>
          This tool will automatically populate your Firestore database with
          sample projects. It will clear existing projects and add{' '}
          {projectsData.length} new projects (
          {projectsData.filter(p => p.isFeatured).length} featured,{' '}
          {projectsData.filter(p => !p.isFeatured).length} grid).
        </p>
          ...
          <div className='mt-6'>
            ...
            <div className='bg-primary-950 rounded p-4 max-h-64 overflow-y-auto border border-primary-700/30'>
              {logs.map((log, index) => (
                <div
                  key={index}
                  className='flex items-start space-x-2 mb-1 text-sm font-mono'
                >
        ...
        <div className='mt-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
            {projectsData.slice(0, 6).map((project, index) => (
              <div
                key={index}
                className='bg-primary-700/30 rounded p-3 border border-primary-600/30'
              >
                <div className='flex items-center justify-between mb-2'>
                  ...
                  {project.isFeatured && (
                    <span className='text-xs bg-accent-500/20 text-accent-500 px-2 py-1 rounded'>
                      Featured
                    </span>
                  )}
                </div>
                ...
                <div className='flex flex-wrap gap-1'>
                  {project.techStack.slice(0, 3).map((tech, techIndex) => (
                    <span
                      key={techIndex}
                      className='text-xs bg-primary-600/50 text-gray-300 px-1 py-0.5 rounded'
                    >
                      {tech}
                    </span>
                  ))}
            ...
            {projectsData.length > 6 && (
              <div className='bg-primary-700/30 rounded p-3 border border-primary-600/30 flex items-center justify-center'>
                <span className='text-gray-400 text-sm'>
                  +{projectsData.length - 6} more projects...
                </span>
              </div>
            )}
          </div>
