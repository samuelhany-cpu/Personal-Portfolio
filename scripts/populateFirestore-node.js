/**
 * Firestore Database Population Script (Node.js CommonJS Version)
 * 
 ...
 * 
 * Usage: node scripts/populateFirestore-node.js
 */

const { initializeApp } = require('firebase/app');
const { getFirestore, collection, addDoc, deleteDoc, getDocs, doc } = require('firebase/firestore');
  ...
  try {
    ...
    const addPromises = projectsData.map(async (project, index) => {
      try {
        ...
        console.log(`✅ Added project ${index + 1}/${projectsData.length}: ${project.title}`);
        return docRef;
      } catch (error) {
...
async function main() {
  ...
  try {
    ...
    console.log('');
    console.log('📊 Summary:');
    console.log(`   • Featured projects: ${projectsData.filter(p => p.isFeatured).length}`);
    console.log(`   • Grid projects: ${projectsData.filter(p => !p.isFeatured).length}`);
